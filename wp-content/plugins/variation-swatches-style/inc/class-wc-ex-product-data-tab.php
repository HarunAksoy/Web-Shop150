...
class WC_EX_Product_Data_Tab_Swatches {
	...
	public function eds_add_my_custom_product_data_tab( $product_data_tabs ) {
		$product_data_tabs['woo-variations-style'] = array(
			'label' => __( 'Variation Swatches', 'atawc_lang' ),
	...
	public function render_product_tab_content() {
		...
		<div class="fields_header ata_wcvs_heading">
			<table class="wcsap widefat">
				...
				<th class="attribute_swatch_label">
					<?php _e( 'Product Attribute Name', 'atawc_lang' ); ?>
				</th>
				<th class="attribute_swatch_type">
					<span><?php _e( 'Attribute Control Type', 'atawc_lang' ); ?></span>
				</th>
				</thead>
			</table>
		</div>
		<div class="fields ">
			...
			if ( $attributes && count( $attributes ) > 0 ) :
									...
									<tr>
										...
										<td class="attribute_swatch_type">
                                            <select class="_swatch_type_options_type" id="_swatch_type_options_<?php echo $key_attr; ?>_type" name="_swatch_type_options[<?php echo $key; ?>][type]">
                                                <option <?php selected( $current_type, 'default' ); ?> value="default"><?php _e( 'None', 'atawc_lang' ); ?></option>
                                                <?php if ( $current_is_taxonomy ) : ?>
                                                    <option <?php selected( $current_type, 'term_options' ); ?> value="term_options"><?php _e( 'Taxonomy Lebel, Colors and Images', 'atawc_lang' ); ?></option>
                                                <?php endif; ?>
                                                <option <?php selected( $current_type, 'product_color' ); ?> value="product_color"><?php _e( 'Custom Colors', 'atawc_lang' ); ?></option>
                                                  <option <?php selected( $current_type, 'product_image' ); ?> value="product_image"><?php _e( 'Custom Images', 'atawc_lang' ); ?></option>
                                                    <option <?php selected( $current_type, 'product_label' ); ?> value="product_label"><?php _e( 'Custom Lebel', 'atawc_lang' ); ?></option>
								 ...
								 ?>
                                    ...
                                    <div class="sub_field field">
                                        ...
                                        <div class="wcsap_field_meta ata_wcvs_meta">
                                                ...
                                                <a href="javascript:void(0)" class="button ata_woo_meta_uploader" data-uploader-title="Add image(s) to  <?php echo esc_attr( $attribute_term['label'] ); ?>" data-uploader-button-text="Add image(s)  <?php echo esc_attr( $attribute_term['label'] ); ?> "> <?php _e( 'Upload/Add image', 'atawc_lang' ); ?></a>
                                                <a ref="javascript:void(0)" class="remove_ata_woo_meta_img button "><?php _e( 'Remove image', 'atawc_lang' ); ?></a>
				...
				echo '<p>' . __( 'Add a at least one attribute / variation combination to this product that has been configured with color swatches or photos. After you add the attributes from the "Attributes" tab and create a variation, save the product and you will see the option to configure the swatch or photo picker here.', 'atawc_lang' ) . '</p>';
			endif;
			?>
